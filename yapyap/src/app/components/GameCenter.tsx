...
export default function GameCenter({ isOpen, onClose, user, friendsList, chats, onOpenChat, onStartGame }: GameCenterProps) {
  const [selectedGame, setSelectedGame] = useState<'tictactoe' | 'rps' | null>(null);
                  ...
                  return (
                    <li key={chat.id} className={styles.notificationItem} onClick={() => { onOpenChat(chat.id); onClose(); }}>
                      ...
                      <div style={{ fontSize: 13, color: 'var(--muted)' }}>
                        {chat.game.type === 'rps' ? 'Rock Paper Scissors' : 'Tic-Tac-Toe'} • {chat.game.turn === user.uid ? 'Your Turn' : 'Their Turn'}
                      </div>
                    </li>
                  );
                })}
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button 
                  ...
                  style={{ flex: 1, background: selectedGame === 'tictactoe' ? 'var(--accent)' : 'rgba(0,0,0,0.05)', color: selectedGame === 'tictactoe' ? 'white' : 'var(--accent-1)' }}
                ...
                <button 
                  className={styles.sendBtn} 
                  style={{ flex: 1, background: selectedGame === 'rps' ? 'var(--accent)' : 'rgba(0,0,0,0.05)', color: selectedGame === 'rps' ? 'white' : 'var(--accent-1)' }}
